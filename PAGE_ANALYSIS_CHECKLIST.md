...
|---------|---------|----------|------------|---------|---------|------|
| 用户列表 | `mng-list-all.php` | ✅ 已完成 | `User` | `/api/v1/users` | `UsersView.vue` | 完整CRUD功能 |
| 用户新建 | `mng-new.php` | ✅ 已完成 | `User` | `POST /api/v1/users` | `UserForm.vue` | 表单验证完整 |
| 用户编辑 | `mng-edit.php` | ✅ 已完成 | `User` | `PUT /api/v1/users/{id}` | `UserForm.vue` | 支持编辑模式 |
| 用户删除 | `mng-del.php` | ✅ 已完成 | `User` | `DELETE /api/v1/users/{id}` | 集成在列表中 | 确认对话框 |
| 快速添加用户 | `mng-new-quick.php` | 🟡 部分完成 | `User` | `POST /api/v1/users/quick` | 需要开发 | 简化表单 |
| 用户搜索 | `mng-search.php` | ✅ 已完成 | `User` | `GET /api/v1/users?search` | 集成在列表中 | 动态搜索 |
| 批量导入 | `mng-import-users.php` | ✅ 已完成 | `User` | `POST /api/v1/users/import` | `UserImportModal.vue` | Excel/CSV导入 |
| 用户详情 | 无独立PHP | ✅ 已完成 | `User` | `GET /api/v1/users/{id}` | `UserDetail.vue` | 新增功能 |
| 在线用户监控 | 集成在其他页面 | ✅ 已完成 | `RadAcct` | `GET /api/v1/users/online` | `OnlineUsersMonitor.vue` | 实时监控 |
