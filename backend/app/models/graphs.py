...
class GraphStatistics(Base):
    ...
    time_period: Mapped[str] = mapped_column(
        String(32), nullable=False)  # daily, weekly, monthly, yearly

    # Time dimensions
    recorded_date: Mapped[date] = mapped_column(
        Date, nullable=False, index=True)
    recorded_hour: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True)  # 0-23 for hourly stats
...
class LoginStatistics(Base):
    ...
    stat_hour: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, index=True)  # For hourly breakdown
    ...
    nas_breakdown: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True)  # NAS IP -> count
    user_agent_breakdown: Mapped[Optional[dict]
                                 ] = mapped_column(JSON, nullable=True)

    # Performance metrics
    avg_response_time: Mapped[Optional[float]
                              ] = mapped_column(Float, nullable=True)
    peak_concurrent_users: Mapped[Optional[int]
...
class TrafficStatistics(Base):
    ...
    avg_session_duration: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # in seconds
    ...
    top_upload_users: Mapped[Optional[dict]
                             ] = mapped_column(JSON, nullable=True)
    top_download_users: Mapped[Optional[dict]
                               ] = mapped_column(JSON, nullable=True)
    top_nas_devices: Mapped[Optional[dict]
                            ] = mapped_column(JSON, nullable=True)
...
class UserStatistics(Base):
    ...
    active_users: Mapped[int] = mapped_column(
        Integer, default=0)  # Users who logged in
    new_users: Mapped[int] = mapped_column(
        Integer, default=0)  # New registrations
    online_users: Mapped[int] = mapped_column(
        Integer, default=0)  # Currently online

    # User activity metrics
    avg_session_time: Mapped[Optional[float]
                             ] = mapped_column(Float, nullable=True)
    avg_daily_usage: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # bytes per user
    user_retention_rate: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # percentage

    # User categorization
    power_users_count: Mapped[int] = mapped_column(
        Integer, default=0)  # Top 10% by usage
    ...
    geographic_distribution: Mapped[Optional[dict]
                                    ] = mapped_column(JSON, nullable=True)
...
class SystemMetrics(Base):
    ...
    radius_requests: Mapped[Optional[int]
                            ] = mapped_column(Integer, nullable=True)
    radius_accepts: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True)
    radius_rejects: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True)
    radius_response_time: Mapped[Optional[float]
                                 ] = mapped_column(Float, nullable=True)

    # Database metrics
    db_connections: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True)
    db_query_time: Mapped[Optional[float]
                          ] = mapped_column(Float, nullable=True)
...
class GraphTemplate(Base):
    ...
    category: Mapped[str] = mapped_column(
        String(64), nullable=False)  # login, traffic, user, system

    # Graph configuration
    graph_type: Mapped[GraphType] = mapped_column(String(32), nullable=False)
    data_source: Mapped[str] = mapped_column(
        String(128), nullable=False)  # API endpoint or data source
    time_granularity: Mapped[TimeGranularity] = mapped_column(
        String(32), nullable=False)

    # Chart configuration
    chart_config: Mapped[dict] = mapped_column(
        JSON, nullable=False)  # Chart.js configuration

    # Display settings
    title: Mapped[str] = mapped_column(String(255), nullable=False)
    subtitle: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    x_axis_label: Mapped[Optional[str]] = mapped_column(
        String(128), nullable=True)
    y_axis_label: Mapped[Optional[str]] = mapped_column(
        String(128), nullable=True)

    # Filtering and grouping
    default_filters: Mapped[Optional[dict]
                            ] = mapped_column(JSON, nullable=True)
    group_by_options: Mapped[Optional[list]
                             ] = mapped_column(JSON, nullable=True)
...
class DashboardWidget(Base):
    ...
    dashboard_id: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True)  # Dashboard identifier
    ...
    border_color: Mapped[Optional[str]] = mapped_column(
        String(16), nullable=True)
...
Index('idx_graph_statistics_type_date',
      GraphStatistics.graph_type, GraphStatistics.recorded_date)
Index('idx_graph_statistics_metric_period',
      GraphStatistics.metric_name, GraphStatistics.time_period)
Index('idx_login_statistics_date_hour',
      LoginStatistics.stat_date, LoginStatistics.stat_hour)
Index('idx_traffic_statistics_date_hour',
      TrafficStatistics.stat_date, TrafficStatistics.stat_hour)
Index('idx_user_statistics_date_hour',
      UserStatistics.stat_date, UserStatistics.stat_hour)
Index('idx_system_metrics_recorded_type',
      SystemMetrics.recorded_at, SystemMetrics.metric_type)
Index('idx_graph_templates_category_active',
      GraphTemplate.category, GraphTemplate.is_active)
Index('idx_dashboard_widgets_dashboard_position', DashboardWidget.dashboard_id,
      DashboardWidget.position_x, DashboardWidget.position_y)
