...
class UpsStatus(Base):
    ...
    # UPS identification
    ups_name: Mapped[str] = mapped_column(
        String(64), nullable=False, index=True)
    ups_model: Mapped[Optional[str]] = mapped_column(
        String(128), nullable=True)
    ups_serial: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True)
    location: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)

    # Power status
    status: Mapped[SystemStatus] = mapped_column(
        SQLEnum(SystemStatus), default=SystemStatus.OFFLINE)
    battery_charge: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # Percentage
    battery_runtime: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True)  # Minutes
    input_voltage: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # Volts
    output_voltage: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # Volts
    load_percentage: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # Percentage

    # Temperature and environment
    temperature: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # Celsius
    humidity: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # Percentage

    # Timestamps
    last_test_date: Mapped[Optional[datetime]
                           ] = mapped_column(DateTime, nullable=True)
    last_battery_replacement: Mapped[Optional[datetime]] = mapped_column(
...
class RaidStatus(Base):
    ...
    # RAID identification
    array_name: Mapped[str] = mapped_column(
        String(64), nullable=False, index=True)
    # RAID0, RAID1, RAID5, etc.
    raid_level: Mapped[str] = mapped_column(String(16), nullable=False)
    controller_name: Mapped[Optional[str]] = mapped_column(
        String(128), nullable=True)
    ...
    # Storage information
    total_size: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True)  # Bytes
    used_size: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True)  # Bytes
    available_size: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True)  # Bytes

    # Performance metrics
    read_rate: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # MB/s
    write_rate: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # MB/s
...
class HeartBeat(Base):
    ...
    response_time: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True)  # Milliseconds
    uptime: Mapped[Optional[int]] = mapped_column(
    ...
    last_response: Mapped[Optional[datetime]
                          ] = mapped_column(DateTime, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime, default=func.now(), onupdate=func.now())
...
class ReportGeneration(Base):
    ...
    date_range_start: Mapped[Optional[datetime]
                             ] = mapped_column(DateTime, nullable=True)
    date_range_end: Mapped[Optional[datetime]
                           ] = mapped_column(DateTime, nullable=True)
...
class ServerMonitoring(Base):
    ...
    uptime: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True)  # Seconds
    load_average: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True)  # 1min, 5min, 15min
    active_connections: Mapped[Optional[int]
                               ] = mapped_column(Integer, nullable=True)

    # Service status
    services_status: Mapped[Optional[dict]
...
Index('idx_report_generation_type_status',
      ReportGeneration.report_type, ReportGeneration.status)
Index('idx_server_monitoring_name_recorded',
      ServerMonitoring.server_name, ServerMonitoring.recorded_at)
