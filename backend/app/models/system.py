...
class SystemConfig(Base):
    ...
    config_type: Mapped[str] = mapped_column(String(32), default="string")  # string, integer, boolean, json
    category: Mapped[str] = mapped_column(String(64), nullable=False, index=True)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    
    # Configuration metadata
    is_encrypted: Mapped[bool] = mapped_column(Boolean, default=False)
    is_system: Mapped[bool] = mapped_column(Boolean, default=False)  # System configs can't be deleted
...
class SystemLog(Base):
    ...
    # Log details
    log_level: Mapped[str] = mapped_column(String(16), nullable=False, index=True)  # DEBUG, INFO, WARNING, ERROR, CRITICAL
    logger_name: Mapped[str] = mapped_column(String(64), nullable=False, index=True)
    ...
    exception_traceback: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class BackupHistory(Base):
    ...
    backup_type: Mapped[str] = mapped_column(String(32), nullable=False)  # full, incremental, differential
    backup_size: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Size in bytes
    ...
    # Backup metadata
    tables_included: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON list of tables
...
class CronJob(Base):
    ...
    # Job details
    job_name: Mapped[str] = mapped_column(String(128), nullable=False, unique=True)
    ...
    last_status: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)  # success, failed, running
    last_output: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    
    # Job metadata
    max_runtime: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Max runtime in seconds
...
class MailQueue(Base):
    ...
    status: Mapped[str] = mapped_column(String(32), default="pending")  # pending, sending, sent, failed
    error_message: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    retry_count: Mapped[int] = mapped_column(Integer, default=0)
    max_retries: Mapped[int] = mapped_column(Integer, default=3)
...
class NotificationTemplate(Base):
    ...
    template_type: Mapped[str] = mapped_column(String(32), nullable=False)  # email, sms, push
    category: Mapped[str] = mapped_column(String(64), nullable=False)  # user_welcome, password_reset, etc.
    
    # Template content
    subject: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # For emails
    body_text: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    body_html: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    
    # Template metadata
    variables: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON list of available variables
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
...
class ApiKey(Base):
    ...
    secret_key: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    
    # Permissions and restrictions
    permissions: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON list of permissions
    allowed_ips: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON list of allowed IPs
...
class DatabaseMaintenance(Base):
    ...
    # Operation details
    operation_type: Mapped[str] = mapped_column(String(32), nullable=False)  # vacuum, reindex, analyze, cleanup
    table_name: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    operation_details: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    
    # Operation results
    status: Mapped[str] = mapped_column(String(32), default="running")  # running, completed, failed
    rows_affected: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    size_before: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Size in bytes
...
class SessionStore(Base):
    ...
    # Session data
    session_data: Mapped[str] = mapped_column(Text, nullable=False)  # JSON encoded session data
    username: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True)
    ...
    # Session metadata
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    last_activity: Mapped[datetime] = mapped_column(DateTime, default=func.now(), index=True)
...
class AuditTrail(Base):
    ...
    action: Mapped[str] = mapped_column(String(16), nullable=False, index=True)  # INSERT, UPDATE, DELETE
