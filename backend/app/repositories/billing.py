...
class BillingMerchantRepository:
    ...
    async def update(self, merchant_id: int, merchant_data: Dict[str, Any]) -> Optional[BillingMerchant]:
        ...
        try:
            ...
            logger.error(f"Error updating merchant transaction {merchant_id}: {str(e)}")
            self.session.rollback()
            raise DatabaseError(f"Failed to update merchant transaction: {str(e)}")
    ...
    async def delete(self, merchant_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Error deleting merchant transaction {merchant_id}: {str(e)}")
            self.session.rollback()
            raise DatabaseError(f"Failed to delete merchant transaction: {str(e)}")
...
class PaymentRepository:
    ...
    async def get_all(
        ...
        try:
            ...
            # Apply filters
            if customer_filter:
                query = query.filter(Payment.customer_id.ilike(f"%{customer_filter}%"))
    ...
    async def get_by_customer(self, customer_id: str) -> List[Payment]:
        ...
        try:
            ...
            logger.error(f"Error fetching payments for customer {customer_id}: {str(e)}")
            raise DatabaseError(f"Failed to fetch payments: {str(e)}")
...
class RefundRepository:
    ...
    async def get_all(
        ...
        try:
            ...
            # Apply filters
            if customer_filter:
                query = query.filter(Refund.customer_id.ilike(f"%{customer_filter}%"))
    ...
    async def get_by_payment(self, payment_id: int) -> List[Refund]:
        ...
        try:
            ...
            logger.error(f"Error fetching refunds for payment {payment_id}: {str(e)}")
            raise DatabaseError(f"Failed to fetch refunds: {str(e)}")
...
class PaymentTypeRepository:
    ...
    async def get_all(
        ...
        try:
            ...
            # Apply sorting
            sort_column = getattr(PaymentType, sort_field, PaymentType.sort_order)
            if sort_order == "desc":
                query = query.order_by(desc(sort_column))
            else:
                query = query.order_by(asc(sort_column))
    ...
    async def get_by_id(self, payment_type_id: int) -> Optional[PaymentType]:
        ...
        try:
            ...
            logger.error(f"Error fetching payment type {payment_type_id}: {str(e)}")
    ...
    async def update(self, payment_type_id: int, payment_type_data: Dict[str, Any]) -> Optional[PaymentType]:
        ...
        try:
            ...
            logger.error(f"Error updating payment type {payment_type_id}: {str(e)}")
            self.session.rollback()
    ...
    async def delete(self, payment_type_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Error deleting payment type {payment_type_id}: {str(e)}")
            self.session.rollback()
...
class POSRepository:
    ...
    async def get_by_serial_number(self, serial_number: str) -> Optional[POS]:
        ...
        try:
            ...
            logger.error(f"Error fetching POS terminal {serial_number}: {str(e)}")
            raise DatabaseError(f"Failed to fetch POS terminal: {str(e)}")
