...
class ReportsRepository:
    ...
    def get_history_report(self, username: Optional[str] = None,
        ...
        if session_time_min:
            query = query.filter(RadAcct.acctsessiontime >= session_time_min * 60)
        
        sessions = query.order_by(desc(RadAcct.acctstarttime)).limit(limit).all()
    ...
    def get_batch_report(self, batch_name: Optional[str] = None,
        ...
        if batch_name:
            query = query.filter(BatchHistory.batch_name.like(f"%{batch_name}%"))
    ...
    def get_system_status_report(self) -> Dict[str, Any]:
        ...
        # Server monitoring
        server_monitoring = ServerMonitoringRepository(self.db).get_all_latest()
