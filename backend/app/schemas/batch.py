...
class BatchHistoryBase(BaseModel):
    ...
    batch_name: str = Field(..., min_length=1, max_length=255, description="Name of the batch operation")
    batch_description: Optional[str] = Field(None, max_length=1000, description="Description of the batch operation")
    hotspot_id: Optional[int] = Field(None, description="Associated hotspot ID")
    operation_type: str = Field(..., min_length=1, max_length=50, description="Type of batch operation")
    operation_details: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional operation details")
    total_count: int = Field(0, ge=0, description="Total number of items in the batch")
    success_count: int = Field(0, ge=0, description="Number of successful operations")
    failure_count: int = Field(0, ge=0, description="Number of failed operations")
...
class BatchOperationRequest(BaseModel):
    ...
    operation_type: str = Field(..., min_length=1, max_length=50, description="Type of batch operation")
    target_ids: List[int] = Field(..., min_items=1, description="List of target IDs for the operation")
    operation_data: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Operation-specific data")
    batch_name: Optional[str] = Field(None, max_length=255, description="Optional name for the batch operation")
    batch_description: Optional[str] = Field(None, max_length=1000, description="Optional description")
    hotspot_id: Optional[int] = Field(None, description="Associated hotspot ID")


class BatchOperationResult(BaseModel):
    """Schema for batch operation results"""
    batch_history_id: int = Field(..., description="ID of the created batch history record")
    operation_type: str = Field(..., description="Type of operation performed")
    total_count: int = Field(..., ge=0, description="Total number of items processed")
    ...
    details: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional operation details")


class BatchUserOperationRequest(BatchOperationRequest):
    """Schema for batch user operations"""
    operation_type: str = Field(..., regex=r'^(create|delete|update|activate|deactivate)$')


class BatchNasOperationRequest(BatchOperationRequest):
    """Schema for batch NAS operations"""
    operation_type: str = Field(..., regex=r'^(delete|update|activate|deactivate)$')


class BatchGroupOperationRequest(BatchOperationRequest):
    """Schema for batch group operations"""
    operation_type: str = Field(..., regex=r'^(add_users|remove_users|delete|update)$')
