...
class HotspotBase(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=200,
                      description="Hotspot name identifier")
    mac: str = Field(..., min_length=1, max_length=200,
                     description="MAC address or IP address")
    geocode: Optional[str] = Field(
        None, max_length=200, description="Geographic coordinates or location code")
    type: Optional[str] = Field(
        None, max_length=200, description="Hotspot type or category")

    # Owner information
    owner: Optional[str] = Field(
        None, max_length=200, description="Owner name")
    email_owner: Optional[EmailStr] = Field(
        None, description="Owner email address")

    # Manager information
    manager: Optional[str] = Field(
        None, max_length=200, description="Manager name")
    email_manager: Optional[EmailStr] = Field(
        None, description="Manager email address")

    # Location and contact details
    address: Optional[str] = Field(
        None, max_length=200, description="Physical address")
    phone1: Optional[str] = Field(
        None, max_length=200, description="Primary phone number")
    phone2: Optional[str] = Field(
        None, max_length=200, description="Secondary phone number")

    # Company information
    company: Optional[str] = Field(
        None, max_length=200, description="Company name")
    companywebsite: Optional[str] = Field(
        None, max_length=200, description="Company website URL")
    companyemail: Optional[EmailStr] = Field(
        None, description="Company email address")
    companycontact: Optional[str] = Field(
        None, max_length=200, description="Company contact person")
    companyphone: Optional[str] = Field(
        None, max_length=200, description="Company phone number")
...
class HotspotResponse(HotspotBase):
    ...
    updateby: Optional[str] = Field(
        None, description="Last updated by operator")
...
class HotspotListResponse(BaseModel):
    ...
    hotspots: List[HotspotResponse] = Field(...,
...
class HotspotSearchRequest(BaseModel):
    ...
    order_type: str = Field('asc', regex='^(asc|desc)$',
...
class HotspotValidationRequest(BaseModel):
    ...
    exclude_id: Optional[int] = Field(
        None, description="ID to exclude from validation")
...
class HotspotValidationResponse(BaseModel):
    ...
    valid: bool = Field(...,
                        description="Whether the value is valid/available")
...
class HotspotBulkDeleteRequest(BaseModel):
    ...
    hotspot_ids: List[int] = Field(..., min_items=1,
                                   description="List of hotspot IDs to delete")
...
class HotspotStatisticsResponse(BaseModel):
    ...
    recent_hotspots: int = Field(...,
                                 description="Hotspots created in last 30 days")
    unique_types: int = Field(...,
                              description="Number of unique hotspot types")
    unique_companies: int = Field(...,
                                  description="Number of unique companies")
    unique_owners: int = Field(..., description="Number of unique owners")
    types_distribution: Dict[str, int] = Field(
        ..., description="Distribution of hotspots by type")
