...
class RadIpPoolBase(BaseModel):
    """Base IP Pool schema"""
    pool_name: str = Field(..., min_length=1, max_length=30, description="IP pool name")
    framedipaddress: IPv4Address = Field(..., description="IP address")
    nasipaddress: IPv4Address = Field(..., description="NAS IP address")
    calledstationid: Optional[str] = Field(None, max_length=30, description="Called station ID")
    callingstationid: Optional[str] = Field(None, max_length=30, description="Calling station ID (MAC)")
    expiry_time: Optional[datetime] = Field(None, description="IP expiry time")
    username: Optional[str] = Field(None, max_length=64, description="Assigned username")
    pool_key: Optional[str] = Field(None, max_length=30, description="Pool key")
...
class ProfileBase(BaseModel):
    """Base Profile schema"""
    profile_name: str = Field(..., min_length=1, max_length=64, description="Profile name")
    description: Optional[str] = Field(None, max_length=255, description="Profile description")


class ProfileCreate(ProfileBase):
    """Schema for creating profiles"""
    check_attributes: List[RadiusAttribute] = Field(default_factory=list, description="Check attributes")
    reply_attributes: List[RadiusAttribute] = Field(default_factory=list, description="Reply attributes")
...
class RealmBase(BaseModel):
    """Base Realm schema"""
    realmname: str = Field(..., min_length=1, max_length=64, description="Realm name")
    type: Optional[str] = Field(None, max_length=64, description="Realm type")
    authhost: Optional[str] = Field(None, max_length=128, description="Authentication host")
    accthost: Optional[str] = Field(None, max_length=128, description="Accounting host")
    secret: Optional[str] = Field(None, max_length=128, description="Shared secret")
    ...
    notrealm: Optional[str] = Field(None, max_length=128, description="No realm")
...
class ProxyBase(BaseModel):
    """Base Proxy schema"""
    proxyname: str = Field(..., min_length=1, max_length=128, description="Proxy name")
    retry_delay: Optional[int] = Field(None, ge=0, description="Retry delay in seconds")
    retry_count: Optional[int] = Field(None, ge=0, description="Number of retries")
    dead_time: Optional[int] = Field(None, ge=0, description="Dead time in seconds")
    default_fallback: bool = Field(default=False, description="Is default fallback")
...
class HuntGroupBase(BaseModel):
    ...
    groupname: str = Field(..., min_length=1, max_length=64, description="Hunt group name")
    nasipaddress: str = Field(..., max_length=15, description="NAS IP address")
    nasportid: Optional[str] = Field(None, max_length=15, description="NAS port ID")
...
class IpPoolStatistics(BaseModel):
    ...
    pools_by_nas: List[Dict[str, Any]] = Field(default_factory=list, description="Pools grouped by NAS")
    top_pools: List[Dict[str, Any]] = Field(default_factory=list, description="Most used pools")
...
class ProfileStatistics(BaseModel):
    ...
    attributes_per_profile: Dict[str, int] = Field(default_factory=dict, description="Attributes per profile")
...
class RealmStatistics(BaseModel):
    ...
    realms_by_type: Dict[str, int] = Field(default_factory=dict, description="Realms by type")
...
class ProxyStatistics(BaseModel):
    ...
    default_fallback_proxies: int = Field(..., description="Default fallback proxies")


class HuntGroupStatistics(BaseModel):
    """Hunt group statistics"""
    total_hunt_groups: int = Field(..., description="Total hunt groups")
    groups_by_nas: Dict[str, int] = Field(default_factory=dict, description="Groups by NAS")
...
class ProfileListResponse(BaseModel):
    ...
    profiles: List[str] = Field(default_factory=list, description="Profile names")
    total: int = Field(..., description="Total count")
...
class HuntGroupListResponse(BaseModel):
    ...
    groups: List[str] = Field(default_factory=list, description="Hunt group names")
    total: int = Field(..., description="Total count")
