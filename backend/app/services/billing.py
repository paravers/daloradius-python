...
class InvoiceService:
    ...
    async def get_invoices(
        ...
        try:
            ...
            # Convert to response models
            invoice_responses = [self._to_response_model(invoice) for invoice in invoices]
    ...
    async def create_invoice(self, invoice_data: InvoiceCreate) -> InvoiceResponse:
        ...
        try:
            ...
            invoice_dict['creationby'] = 'system'  # Should come from auth context
...
class PaymentService:
    ...
    async def get_payments(
        ...
        try:
            ...
            # Convert to response models
            payment_responses = [self._to_response_model(payment) for payment in payments]
    ...
    async def create_payment(self, payment_data: PaymentCreate) -> PaymentResponse:
        ...
        try:
            ...
            payment_dict['creationby'] = 'system'  # Should come from auth context
...
class RefundService:
    ...
    async def get_refunds(
        ...
        try:
            ...
            # Convert to response models
            refund_responses = [self._to_response_model(refund) for refund in refunds]
    ...
    async def create_refund(self, refund_data: RefundCreate) -> RefundResponse:
        ...
        try:
            ...
            refund_dict['creationby'] = 'system'  # Should come from auth context
...
class PaymentTypeService:
    ...
    async def get_payment_types(
        ...
        try:
            ...
            # Convert to response models
            payment_type_responses = [self._to_response_model(payment_type) for payment_type in payment_types]
    ...
    async def get_payment_type(self, payment_type_id: int) -> PaymentTypeResponse:
        ...
        try:
            ...
            if not payment_type:
                raise NotFoundError(f"Payment type with ID {payment_type_id} not found")
    ...
    async def create_payment_type(self, payment_type_data: PaymentTypeCreate) -> PaymentTypeResponse:
        ...
        try:
            ...
            if existing:
                raise ValidationError(f"Payment type with code '{payment_type_data.code}' already exists")
            
            # Set creation metadata
            payment_type_dict = payment_type_data.model_dump(exclude_unset=True)
            payment_type_dict['creationdate'] = datetime.utcnow()
            payment_type_dict['creationby'] = 'system'  # Should come from auth context
            ...
            raise BusinessLogicError(f"Failed to create payment type: {str(e)}")
    ...
    async def update_payment_type(self, payment_type_id: int, payment_type_data: PaymentTypeUpdate) -> PaymentTypeResponse:
        ...
        try:
            ...
            if 'code' in update_data and update_data['code'] != existing_payment_type.code:
                ...
                if existing:
                    raise ValidationError(f"Payment type with code '{update_data['code']}' already exists")
            ...
            raise BusinessLogicError(f"Failed to update payment type: {str(e)}")
    ...
    async def delete_payment_type(self, payment_type_id: int) -> bool:
        ...
        try:
            ...
            raise BusinessLogicError(f"Failed to delete payment type: {str(e)}")
...
class POSService:
    ...
    async def get_terminals(
        ...
        try:
            ...
            # Convert to response models
            terminal_responses = [self._to_response_model(terminal) for terminal in terminals]
    ...
    async def create_terminal(self, pos_data: POSCreate) -> POSResponse:
        ...
        try:
            ...
            if existing:
                raise ValidationError(f"POS terminal with serial number '{pos_data.serial_number}' already exists")
    ...
    async def update_terminal(self, pos_id: int, pos_data: POSUpdate) -> POSResponse:
        ...
        try:
            ...
            if 'serial_number' in update_data and update_data['serial_number'] != existing_terminal.serial_number:
                ...
                if existing:
                    raise ValidationError(f"POS terminal with serial number '{update_data['serial_number']}' already exists")
