...
class DashboardService:
    ...
    async def _get_traffic_statistics(self) -> Dict[str, Any]:
        ...
        month_start = datetime(datetime.utcnow().year,
                               datetime.utcnow().month, 1)
        month_traffic = await self.accounting_repo.get_traffic_since(month_start)

        return {
            'today_traffic_gb': round(
                (today_traffic.get('input_octets', 0) +
                 today_traffic.get('output_octets', 0)) / (1024**3),
                2
            ),
            'week_traffic_gb': round(
                (week_traffic.get('input_octets', 0) +
                 week_traffic.get('output_octets', 0)) / (1024**3),
                2
            ),
            'month_traffic_gb': round(
                (month_traffic.get('input_octets', 0) +
                 month_traffic.get('output_octets', 0)) / (1024**3),
                2
            ),
            'total_traffic_tb': round(
                (total_traffic.get('input_octets', 0) +
                 total_traffic.get('output_octets', 0)) / (1024**4),
                2
            )
        }
        ...
        try:
            ...
            return {
                ...
                'traffic_last_hour_gb': round(
                    (traffic_last_hour.get('input_octets', 0) +
                     traffic_last_hour.get('output_octets', 0)) / (1024**3),
                    2
                ),
                'failed_logins_today': failed_logins_today,
    ...
    async def get_system_alerts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # >10% failure rate
            if total_logins > 0 and (failed_logins / total_logins) > 0.1:
                alerts.append({
                    'id': 2,
                    'severity': 'warning',
