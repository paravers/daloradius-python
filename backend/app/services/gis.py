...
class GisService:
    ...
    def search_hotspots_near_location(self,
        ...
        hotspots = self.repository.get_hotspots_by_coordinates(
    ...
    def update_hotspot_location(self,
        ...
        hotspot = self.repository.update_hotspot_coordinates(
            hotspot_id, latitude, longitude, updated_by)
    ...
    def remove_hotspot_location(self,
        ...
        Raises:
            NotFoundError: If hotspot not found
        """
        hotspot = self.repository.remove_hotspot_coordinates(
            hotspot_id, updated_by)
    ...
    def get_hotspots_without_location(self,
        ...
        hotspots = self.repository.get_hotspots_without_coordinates(
            page_size, offset)
    ...
    def get_regional_statistics(self,
        ...
        if not (south < north and west < east):
            raise ValidationError(
                "Invalid bounding box: south must be < north, west must be < east")
    ...
    def search_hotspots_by_name(self,
        ...
        hotspots = self.repository.search_hotspots_by_location_name(
            location_query.strip())

        # Filter to only include hotspots with coordinates if requested
        if include_coordinates_only:
            hotspots = [
                hs for hs in hotspots if hs.geocode and self._parse_geocode(hs.geocode)]
    ...
    def update_hotspot_from_ip(self,
        ...
        if not ip_location or not ip_location.get('coordinates'):
            raise ValidationError(
                f"Unable to determine location for IP address: {ip_address}")

        coordinates = ip_location['coordinates']
        if not coordinates.get('latitude') or not coordinates.get('longitude'):
            raise ValidationError(
                "IP geolocation did not return valid coordinates")
