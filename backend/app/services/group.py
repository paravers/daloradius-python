        ...
        for attr_data in attributes:
            try:
                ...
                if not all([attribute, value]):
                    results["errors"].append(f"Missing required fields for attribute: {attr_data}")
                    results["failed"] += 1
                    continue
                ...
                if not is_valid:
                    results["errors"].append(f"Invalid attribute '{attribute}': {message}")
                    results["failed"] += 1
                    continue
                ...
                if success:
                    ...
                    results["failed"] += 1
                    results["errors"].append(f"Failed to create {attribute}: {result}")
                    
            except Exception as e:
                results["failed"] += 1
                results["errors"].append(f"Exception creating attribute {attr_data}: {str(e)}")
        
        results["success_rate"] = results["created"] / results["total_requested"] if results["total_requested"] > 0 else 0
