...
class NasService:
    ...
    async def create_nas(
        ...
        if not self._validate_nas_name(nas_data.nasname):
            raise ValueError(
                "Invalid NAS name format. Must be IP address or valid hostname")
        ...
        asyncio.create_task(
            self._perform_initial_connectivity_test(nas_device.id))
    ...
    async def get_nas_paginated(
        ...
        nas_devices, total = await self.nas_repo.get_paginated(
            ...
            search_fields=['nasname', 'shortname',
                           'description', 'server'] if search else None,
            search_term=search,
            filters=filters or {},
            order_by=sort_by,
            ...
            try:
                ...
                if await self.has_active_sessions(nas_id):
                    ...
                    result["errors"].append(
                        f"NAS {nas_id} has active sessions")
                    continue
                ...
                result["failed_count"] += 1
                result["errors"].append(
                    f"Error deleting NAS {nas_id}: {str(e)}")
    ...
    async def get_nas_statistics(self) -> Dict[str, Any]:
        ...
        total_active_sessions = sum(nas["active_sessions"]
                                    for nas in nas_with_sessions)
        ...
        utilization = (total_active_sessions / total_ports *
                       100) if total_ports else 0
        ...
        for nas in all_nas:
            ...
            type_distribution[nas_type] = type_distribution.get(
                nas_type, 0) + 1
    ...
    async def _perform_initial_connectivity_test(self, nas_id: int):
        ...
        try:
            ...
            logger.warning(
                f"Initial connectivity test failed for NAS {nas_id}: {str(e)}")
